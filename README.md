...
| 📦 **容器执行命令** | 安全的 `docker exec` 工具，仅 admin 可用，支持容器 scope 限制 |
...
| 🚫 **执行任意命令** | 默认不提供 `docker exec`，需在设置中启用并配合容器 scope 限制 |
...
- 🔧 **内置工具**：可以在工具容器里装各种排障工具（curl、dig、nc、docker CLI 等）
...
1. 启动工具容器（示例配置在 `docker/docker-compose-toolbox.yml`）：
   ```bash
...
> 💡 **提示**：工具容器默认挂载了 Docker socket（只读），所以容器内可以直接用 `docker` 命令操作宿主机的 Docker 环境。如果不需要，可以去掉这个挂载。
